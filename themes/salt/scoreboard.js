...
function process(data){
	...
	if(sv.game != data.game){
		$("#main").removeClass("game"+sv.game);
		sv.game = data.game;
		$("#main").addClass("game"+sv.game);
	}
...
function insertPlayerName(player, data){
	...
	if(data.playstyle == "doubles"){
		value = data.player[player].nickname+ ' <span class="sep">/</span> '+data.player[player+"2"].nickname;
	}
...
function insertCharacter(player, data){
	var character = "";
	var dir = "./../assets/characters/stock/"+sv.game+"/";
	...
	$("#pc"+player).css("background-image","url('"+character+"')").attr("data-charid", data.player[player].character.cid);
}
